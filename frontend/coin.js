...
async function consultar() {
  ...
  try {
    ...
    // Salva no histórico local + backend
...
// 🔥 Salva conversão no localStorage + BACKEND
async function salvarNoHistorico(valor, moeda, resultado, taxa) {
  ...
  console.log('Histórico salvo local:', entrada);
  ...
  try {
    ...
    if (resp.ok) {
      ...
      console.error("Erro ao salvar no MongoDB:", await resp.text());
    }
  } catch (err) {
    console.error("Falha ao enviar ao backend:", err);
  }
